...
class Node:
    ...
    def __lt__(self, other: 'Node') -> bool:
        ...
        return sorted(self.symbol) < sorted(other.symbol)
...
def Huffman_tree(symbol_with_probs: dict) -> Node:
    ...
    for symbol in symbols:
        node = Node(symbol_with_probs[symbol], symbol)
        heapq.heappush(nodes_queue, node)

    while len(nodes_queue) > 1:
        first_node = heapq.heappop(nodes_queue)
        first_node.code = "0"

        second_node = heapq.heappop(nodes_queue)
        second_node.code = "1"

        node = Node(roundToDecimals(first_node.prob + second_node.prob, TOL_DEC),
                    "".join(sorted(first_node.symbol + second_node.symbol)))
        node.left = first_node
        node.right = second_node
        heapq.heappush(nodes_queue, node)
