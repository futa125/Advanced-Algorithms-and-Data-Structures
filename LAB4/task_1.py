...
class IntervalTree:
    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        Returns:
            list: List of nodes within the window.
        """
        nodes_within_window = set()

        if is_in_range(qx1, qx2, node.x_median):
            intervals = [*self.filter_intervals(node.intervals, None, qx2, qy1, qy2),
                         *self.filter_intervals(node.intervals, qx1, None, qy1, qy2)]

            for interval in intervals:
                nodes_within_window.add(interval)

            move = "both"

        elif qx1 and node.x_median < qx1:
            intervals = self.filter_intervals(node.intervals, qx1, None, qy1, qy2)
        ...
        elif qx2 and node.x_median > qx2:
            intervals = self.filter_intervals(node.intervals, None, qx2, qy1, qy2)
        ...
        if node.left_child and move in ["both", "left"]:
            nodes_within_window = nodes_within_window.union(self._query(node.left_child, qx1, qx2, qy1, qy2))
        if node.right_child and move in ["both", "right"]:
            nodes_within_window = nodes_within_window.union(self._query(node.right_child, qx1, qx2, qy1, qy2))

        return list(nodes_within_window)

    @staticmethod
    def filter_intervals(intervals, qx1, qx2, qy1, qy2):
        output = []

        for interval in intervals:
            (x1, x2), y = interval
            if qx2 and is_in_window((x1, y), qx1, qx2, qy1, qy2):
                output.append(interval)
            elif qx1 and is_in_window((x2, y), qx1, qx2, qy1, qy2):
                output.append(interval)
